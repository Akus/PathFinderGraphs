...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// Matrix multiplication: NOT BAD BUT MISS SAME DATA
		int sum = 0;
		// creating the matrix
		/*	
		int[][] squareM = new int[getNumVertices()][getNumVertices()];

		for (int i = 0; i < getNumVertices(); i++) {
			for (int j = 0; j < getNumVertices(); j++) {
				squareM[i][j] = 0;
				for (int k = 0; k < getNumVertices(); k++) {
					squareM[i][j] += adjMatrix[i][k] * adjMatrix[k][j];
				}
			}
		}
		...
		*/
		 ...
		 for (int item : getNeighbors(v)) {
		  
		 twoHops.addAll(getNeighbors(item)); }
